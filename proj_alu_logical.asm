...
#####################################################################
au_logical:
	# Caller RTE store: Save $fp, $ra, $a0, $a1, and $a2
	addi	$sp, $sp, -24
	sw	$fp, 24($sp)
	sw	$ra, 20($sp)
	sw	$a0, 16($sp)
...
add_sub_logical:
	# Frame: Save $fp, $ra, $a0, $a1, $a2, $s0, $s1, $s2, $s3, and $s4
	addi	$sp, $sp, -44
	sw	$fp, 44($sp)
	sw	$ra, 40($sp)
	sw	$a0, 36($sp)
	...
	# Code implementation:
	li	$s0, 0				# $s0 = i = 0
	li	$v0, 0				# $v0 = s = Operation result
	li	$t9, 0
	extract_nth_bit($s1, $a2, $t9)		# $s1 = c = First bit in $a2
	
	beq	$s1, 0, compute			# Verify if sub or add
	not	$a1, $a1
...
compute: 
	...
	and 	$t1, $s2, $s3			# $t1 = A.B
	or	$s1, $t0, $t1			# $s1 = c = CI.(A xor B) + A.B = Next carry bit 
	
	insert_to_nth_bit($v0, $s0, $s4, $t9)	# $v0 = y = s[i]
...
twos_complement:
	...
	not	$a0, $a0
	li	$a1, 1
	jal	add_logical	# Arguments in $a0 and $a1: Results in $v0
				# $v0 = $a0 + $a1 = inv($a0) + 1
...
twos_complement_64bit:
	# Frame: Save $fp, $ra, $a0, $a1, $s0, and $s1
	addi	$sp, $sp, -28
	sw	$fp, 28($sp)
	sw	$ra, 24($sp)
	sw	$a0, 20($sp)
	...
	li	$a1, 1
	jal 	add_logical	# Args in $a0 and $a1: Results in $v0 and $v1
...
mul_unsigned:
	# Frame: Save $fp, $ra, $a0, $a1, $s0, $s1, $s2, $s3, and $s4
	addi	$sp, $sp, -40
	sw	$fp, 40($sp)
	sw	$ra, 36($sp)
	sw	$a0, 32($sp)
...
mul_unsigned_loop:
	li	$t9, 0				# $a0 = l[0]
	extract_nth_bit($a0, $s2, $t9)		# Arg in $a0: Results in $v0
	...
	move	$a0, $s1		
	move	$a1, $t0
	jal 	add_logical			# Arg in $a0 and $a1: Results in $v0 and $v1
...
mul_signed:
	# Frame: Save $fp, $ra, $a1, $a2, $s0, $s1, $s2, $s3, and $s4
	addi	$sp, $sp, -40
	sw	$fp, 40($sp)
	sw	$ra, 36($sp)
	sw	$a0, 32($sp)
	...
	jal	twos_complement_if_neg	# Argument in $a0: Results in $v0
	move	$s0, $v0		# $s0 = N1
	...
	move	$a0, $s0
	move	$a1, $s1
	jal 	mul_unsigned		# Arguments in $a0 and $a1: Results in $v0 and $v1
	
	bne 	$s4, 1, mul_signed_end
	move	$a0, $v0
	move	$a1, $v1
	jal	twos_complement_64bit	# Arguments in $a0 and $a1: Results in $v0 and $v1
...
div_unsigned: 
	# Frame: Save $fp, $ra, $a0, $a1, $s0, $s1, $s2, and $s3
	addi	$sp, $sp, -36
	sw	$fp, 36($sp)
	sw	$ra, 32($sp)
	sw	$a0, 28($sp)
...
div_unsigned_loop:
	...
	move	$a0, $s3			# $a0 = r
	move	$a1, $s2			# $a1 = d
	jal 	sub_logical 			# Args in $a0 and $a1: Results in $v0
						# $v0 = s = r - d 
...
div_signed:
	# Frame: Save $fp, $ra, $a0, $a1, $s0, $s1, $s2, $s3, $s4, and $s5
	addi	$sp, $sp, -48
	sw	$fp, 48($sp)
	sw	$ra, 44($sp)
	sw	$a0, 40($sp)
	...
	move	$a0, $s0
	move	$a1, $s1
	jal	div_unsigned			# Args in $a0 and $a1: Results in $v0 and $v1
	move	$s4, $v0			# $s4 = $v0 = Q, $v1 = R
	move	$s5, $v1

	bne 	$s3, 1, check_sign_of_R
	move	$a0, $s4	
	jal	twos_complement			# Args in $a0: Results in $v0 = twos complement of Q
	move	$s4, $v0
	
check_sign_of_R:
	bne	$s2, 1, div_signed_end
	move	$a0, $s5
	jal	twos_complement			# Args in $a0: Results in $v0 = twos complement of R
	move	$s5, $v0
